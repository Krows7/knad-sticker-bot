...
public class StickerRendererTest {
	...
	private static BufferedImage makeSample(String name, String text) {
		...
		StickerRenderer r = new StickerRenderer(
				new StickerData(timestamp, name, text, color, OptionalPair.of(name.toUpperCase().charAt(0) + "")));
		r.renderMessage();
		return r.fitImage();
	}
	...
	public static void main(String[] args) throws IOException {
		...
		File sampleFile = Utils.getOutputFile("tmp.png");
		ImageIO.write(sampleImage, "png", sampleFile);

		if (Utils.contains(args, "show"))
			Desktop.getDesktop().open(sampleFile);
		if (Utils.contains(args, "copy"))
			Toolkit.getDefaultToolkit().getSystemClipboard().setContents(new TransferableImage(sampleImage), null);
	}
}
